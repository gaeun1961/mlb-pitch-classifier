...
│   └── retrain_model.yml  # 수동 트리거 시 재학습 후 새 모델을 PR로 올리는 워크플로우
│
...
- **자연어 설명 + 안전한 폴백**: 예측 구종·신뢰도·2순위 구종·투구 손을 반영한 프롬프트로 Gemini API가
  ...
  규칙 기반 설명으로 즉시 대체해 `/predict`가 항상 응답. Streamlit 앱에서는 이 호출(최대 15초
  타임아웃)을 백그라운드 스레드로 분리해 실행 — 슬라이더·스트라이크존 클릭은 항상 즉시 반응하고,
  설명은 준비되는 대로 자동 새로고침 프래그먼트가 채워 넣음 (아래 설계 결정 참고)
...
- **즐겨찾기**: 검색한 투수를 별 버튼으로 즐겨찾기하면 이름을 다시 안 쳐도 원클릭 재검색 가능(최대 10명).
  로그인 기능이 없어 서버에 저장하지 않고 세션에만 유지 — 브라우저 탭을 닫으면 초기화됨
...
- **일일 Statcast 자동 수집**: GitHub Actions가 매일 KST 06:00에 전날 리그 전체 투구 데이터를
  ...
  제한(100MB)을 넘지 않도록 함
- **재학습 자동화(수동 트리거)**: `retrain_model.yml`을 실행하면 데이터 다운로드부터 학습·평가·
  아티팩트 교체까지 자동으로 하고, 결과를 바로 `backend/model/`에 반영하는 대신 정확도·weighted F1·
  구종별 리포트를 담은 PR로 올린다. 스케줄이 아니라 수동 트리거인 이유는 새 모델이 항상 더
  낫다는 보장이 없어서(2024+2025 재학습 때 정확도가 94.2%→93.36%로 내려간 적 있음) — 병합 여부는
  사람이 지표를 보고 결정
- **파이프라인 실패 알림**: 일일 수집·재학습 워크플로우가 실패하면 GitHub 이슈를 자동 생성(이미
  열려 있으면 코멘트만 추가)해 조용히 죽는 것을 방지
...
- **왜 자연어 설명을 백그라운드 스레드로 분리했는가?**
  원래는 슬라이더를 움직이거나 스트라이크존을 클릭할 때마다 예측과 Gemini 설명 생성을 한 번에 동기 호출했다. 로컬에서는 안 느껴졌지만, 배포 환경에서 왕복 지연이 조금만 있어도 인터랙션이 멈춘 것처럼 느껴지고, 그 사이 다시 조작하면 두 번째 조작에서야 반영된 것처럼 보이는 문제로 이어졌다(Gemini 호출 타임아웃이 최대 15초). 처음엔 버튼 클릭 시에만 설명을 생성하도록 분리했다가, 슬라이더 반응성은 유지하면서 자동 생성도 원해 `ThreadPoolExecutor`로 백그라운드 실행 + `st.fragment(run_every=1)`로 완료 여부만 폴링하는 방식으로 정착했다.

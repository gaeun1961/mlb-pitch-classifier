...
가천대학교 영상처리 수업 과제로 시작해, 데스크톱 GUI → 웹 데모 → FastAPI 백엔드 분리 → Gemini 기반 자연어 설명 → 실제 MLB 투수 검색까지 단계적으로 확장했습니다.
...
| 모델 접근 | 없음, FastAPI 백엔드를 HTTP로 호출 | 직접 로딩 (TensorFlow) | 직접 로딩 (TensorFlow) |
...
- **투수 이름 자동완성 검색**: 2글자 이상 입력하면 MLB 공식 Stats API로 실시간 부분 검색 →
  포지션(투수)만 필터링 → 소속팀·활동 기간으로 동명이인 구분 → 선택 즉시 좌완/우완과 예측이
  자동 반영. Statcast 데이터 제공 시점(2015년)에 맞춰 조회 가능한 시즌만 표시
  (전 커리어가 2015년 이전인 선수는 조회 불가 안내)
- **구종 사용 비율 미리보기**: 투수 선택 시 해당 시즌 실제 투구의 구종별 사용 비율을 예측 전에 먼저 보여줌
- **콜드 스타트 대응**: Render 무료 플랜의 슬립/웨이크업 특성에 맞춰 첫 요청에는 별도 안내와 로딩 스피너를 표시
...
**왜 MLP인가?**
투구 데이터는 구속·회전수·가속도 등 17개의 수치형 피처로 구성된 정형 데이터라, 공간 패턴을 보는 CNN이나 시계열을 보는 RNN보다 MLP가 더 적합하다. Statcast 피처 자체가 이미 물리적으로 의미 있는 값들로 정제되어 있어 깊은 특징 추출 없이도 Weighted F1 0.94가 나왔다.

**왜 BatchNorm + Dropout인가?**
7개 구종 간 샘플 수 불균형(FF 22,938건 vs FS 2,238건)이 존재한다. BatchNorm은 각 층의 입력 분포를 안정시켜 수렴을 빠르게 하고, Dropout은 특정 구종에 과적합되는 것을 방지하기 위해 함께 적용했다.
...
**왜 FastAPI로 백엔드를 분리했는가?**
초기에는 Streamlit이 모델을 직접 로드했는데, 모델 서빙 로직이 UI 코드와 강하게 결합되어 있었다. FastAPI로 분리하면 모델 교체나 설명 로직 변경이 웹 UI 배포와 독립적으로 이루어지고, 다른 클라이언트에서도 같은 `/predict` 엔드포인트를 재사용할 수 있다.
